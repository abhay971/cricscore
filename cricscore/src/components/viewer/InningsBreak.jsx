 ...
 */
const InningsBreak = ({ match, innings, onStartNextInnings, isSuperOver = false }) => {
  ...
  const badgeText = isSuperOver ? 'SUPER OVER BREAK' : 'INNINGS BREAK';
  const headerText = isSuperOver ? 'Super Over 1st Innings' : '1st Innings Summary';
  const buttonText = isSuperOver ? 'Start SO 2nd Innings' : 'Start 2nd Innings';
    ...
    <div className="fixed inset-0 z-50 bg-[#1A1B2E] flex items-center justify-center p-5 overflow-y-auto">
      ...
      >
        ...
        >
          ...
          <div className="p-8 text-center border-b border-[#4A4B5E]/30">
            ...
            <p className="text-white/70 text-lg">
              ({innings?.totalOvers ?? 0}/{isSuperOver ? 1 : match?.overs} overs)
            </p>
          </div>
        ...
        >
          ...
          <p className="text-white/90 text-sm mt-4">Required Run Rate: {isSuperOver ? target.toFixed(2) : (match?.overs ? (target / match.overs).toFixed(2) : '0.00')}</p>
        </motion.div>

        {/* Start Button - Pill shaped (only for scorer) */}
        {onStartNextInnings && (
          <motion.div
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
